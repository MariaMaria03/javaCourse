...
public class Box extends Container {
    ...
    public void pullItem(int numP) throws ItemsException {
        ArrayList<Item> pileColl = piles.get(numP);
        if (pileColl.isEmpty()) {
            throw new ItemsException("Стопка пустая");
        }
        else {
            final int n = pileColl.size();
